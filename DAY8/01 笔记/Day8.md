...
flex布局
	...
	侧轴对齐方式
        align-items
            center
            stretch
        align-self某个子元素侧轴对齐方式
	子元素空间分配
        flex:数字代表占据父控件剩余尺寸的份数 
    子元素换行
		flex-wrap
	换行之后，行的显示的方式
        align-content
...
flex-grow
	定义项目的放大比例，默认为0，即如果父元素存在刺余空间，也不放大
flex-shrink
	定义项目的缩小比例，默认为1，即如果父元素空间不足，项目将会缩小
flex-basis
...
```
重要性
    1,顺序不对的话，有可能会影响页面布局(1ine-height和font连写冲突)
...
属性书写的一般属性
    ...
    4,点缀属性
        border-radius阴影(text-shadow)光标小手(cursor)	
```

### 选择器使用

```
推荐使用类+后代选择器一般后代选择器层级不要超过3个
```
...
```
1,创建文件夹se1f-intro项目名
2,创建css文件夹用于存放css文件，images文件夹用于存放图片，index.html用来书写htm1页面
