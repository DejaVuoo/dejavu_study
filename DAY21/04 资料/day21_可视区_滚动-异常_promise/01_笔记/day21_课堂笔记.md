...
方法
	...
	element.clientHeight 返回内容的可视高度（包含内边距，不包含边框，外边距和滚动条）
	element.clientWidth 返回内容的可视宽度（包含内边距，不包含边框，外边距和滚动条）
```
...
两种处理方式
	1,抛出(甩锅)  
		throw关键字 可以手动抛出,如果不处理的话,程序也会抛出错误,后续的代码无法执行
	2,处理(接盘)
		try..catch 处理异常,处理之后,后续的代码可以正常执行,提高程序健壮性
