...
互转
    ...
    display:block
    块级->行级display:inline
    行内块display:in]ine-b1ock(可以设置宽高，可以在)
...
```
概念
	伪元素在htm1骨架中，并没有通过代码去实现，而是通过css模拟出来的
作用
	一般在页面中非主体内容可以使用伪元素
区别
	普通元素：通过html标签生成
    伪元素：通过css模拟出来的标签效果
    本质区别：是否在htm1中创建了新的元素
语法
    ::before在父元素位置的最前面添加一个伪元索
    ::after在父元素位置的最后面添加一个
注意
    1,必须设置content属性才能生效
    2,伪元素默认是行内元素    
